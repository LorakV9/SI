...
class Model(nn.Module):
    def __init__(self, input_dim, output_dim, K1, K2, activation_function=nn.ReLU):
...
# Define learning rates and neuron counts
...
for activation_function in activation_functions:
    for k1_ind in range(len(K1_vec)):
        for k2_ind in range(len(K2_vec)):
            model = Model(X_train.shape[1], len(target_names), K1_vec[k1_ind], K2_vec[k2_ind], activation_function=activation_function)
            ...
            with torch.no_grad():
                ...
                print(f"Activation: {activation_function.__name__}, K1: {K1_vec[k1_ind]}, K2: {K2_vec[k2_ind]}, PK: {PK:.2f}%")
    ...
    ax.set_ylabel('K2')
    ax.set_zlabel('PK')
    ax.set_title(f'Activation Function: {activation_function.__name__}')
    ax.view_init(30, 200)
    plt.savefig(f"Fig4_PK_K1K2_{activation_function.__name__}_pytorch_iris.png", bbox_inches='tight')
    plt.show()
